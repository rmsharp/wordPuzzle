...
\title{Word Puzzle for Matthew 26-28}
\subtitle{}
\author{R. Mark Sharp}
\date{\today}
...
@
<<make-puzzle>>=
filename <- "../inst/extdata/word_lists/JesusKilled.txt"
puzzle_list <- make_puzzle(filename, upper_case = TRUE)
@
\clearpage
\Large
\section*{Word Puzzle for Matthew 26-28}
\begin{verbatim}
...
\normalsize
\begin{verbatim}
<<words-rows, include = TRUE, results = 'asis'>>=
